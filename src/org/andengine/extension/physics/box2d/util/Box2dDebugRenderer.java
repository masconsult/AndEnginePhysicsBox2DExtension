	...
	public void onManagedUpdate(float pSecondsElapsed) {
		...
		while(iterator.hasNext()) {
			...
			for(int i = 0; i < shapesSize; i++) {
				...
				final Vector2 linearVelocity = body.getLocalVector(body.getLinearVelocityFromLocalPoint(body.getMassData().center));
				((Line)shape.getChild(0)).setPosition(body.getMassData().center.x * ptm, body.getMassData().center.y * ptm, linearVelocity.x * ptm, linearVelocity.y * ptm);
			}
		}
	...
	private void addBodyToShapeCollection(Body body) {
		...
		for(Fixture fixture : body.getFixtureList()) {
			...
			if(debugShape != null) {
    			...
    			final Line linearVelocity = new Line(body.getMassData().center.x, body.getMassData().center.y, body.getMassData().center.x, body.getMassData().center.y, mVertexBufferObjectManager);
    			linearVelocity.setColor(255, 255, 255);
    			debugShape.attachChild(linearVelocity, 0);
			}
		}
	}
